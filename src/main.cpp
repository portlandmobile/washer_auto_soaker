...
constexpr int TOUCH_PIN = 4;     // Touch sensor output

// Timing (ms)
constexpr unsigned long TEN_MIN   = 600000UL;   // 10 * 60 * 1000
constexpr unsigned long TWO_HOURS = 7200000UL;  // 2 * 60 * 60 * 1000
...
void pulseServo90() {
  myServo.write(49);
  delay(1000);          // allow move to 45° (SG90 ~0.12s/60°, 500ms is safe)
  myServo.write(0);
...
void loop() {
  ...
  if (touch == HIGH && lastTouch == LOW && (now - lastTouchEdgeAt) > DEBOUNCE_MS && !inprogress) {  
    ...
    if (!sequenceStarted) {
      delay (2000); //Small delay in hope that it will help with the sudden draw of power.
